...
% _V_: Polyhedral volume enclosed by _N_
  ...
  if all(isfinite(bounds))
    initRange(:, 2 * NUM + 1 : 3 * NUM) ...
      = initRange(:, 2 * NUM + 1 : 3 * NUM) + mean(bounds);
  elseif isfinite(bounds(1))
    initRange(:, 2 * NUM + 1 : 3 * NUM) ...
      = initRange(:, 2 * NUM + 1 : 3 * NUM) + bounds(1) + halfRange;
  elseif isfinite(bounds(2))
    initRange(:, 2 * NUM + 1 : 3 * NUM) ...
      = initRange(:, 2 * NUM + 1 : 3 * NUM) + bounds(2) - halfRange;                                  
  end
  ...
  objFunc = @(N) -chainlink(N, NUM, maxTL, bounds, FG);   % Create function handle for GA
